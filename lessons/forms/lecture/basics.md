...
Посмотрев на этот подход невольно снова задаешься вопросом: а зачем все таки нужен FormArray, ведь я могу точно так же вручную создавать инстанс нового FormControl и добавлять в FormGroup?

Самая главная проблема в том, как хранить эти контролы. Что такое FormGroup? Это словарь, где ключ это имя форм-контрола, а значение - сам форм-контрол. Представим следующую ситуацию: нужно дать пользователю возможность добавить произвольное количество навыков в профиль. По-умолчанию у нас 2 навыка на форме, когда она инициализируется. 
...
Юзер нажимает на кнопку "Добавить скилл". И вот тут начинаются проблемы:

1. Какой ключ будет у нового форм-контрола? Понятное дело что `skill3` (хотя кстати не факт, ведь можно назвать `anotherOneSkill`). Но откуда нам брать кол-во уже существующих скиллов? Их же нет нигде, а в существующем FormGroup есть только ключи, в которых присутствует порядковый номер скила, по которому мы косвенно можем определить номер последнего скилла. Звучит очень ненадежно. Значит для большей надежности нам придется создать поле для хранения кол-во скиллов. А это значит что его надо будет каждый раз обновлять, сбрасывать и тд. Как то неудобно, не так ли?
2. Ну допустим мы сделаем это, мы же умеем писать код! Теперь представим что у нас добавлено 7 скиллов. И как то так мы решили что мы не хотим больше иметь в списке скилл под номером 4. Хотим его удалить. То есть его надо будет удалить из FormGroup, с указанием соответствующего порядкового номера. Тоже как то не очень, ведь все снова зависит от имени контрола, которое может поменяться когда угодно.
3. Плюсом к этому добавляется потенциальные сложности с обходом контролов в HTML: по какой коллекции у нас будет создаваться цикл `@for` в разметке, ведь FormGroup это не массив? Снова надо че то придумывать, мапить тд.

А теперь представьте что у нас таких динамических полей будет 10. Для всех них писать подобную логику? Попахивает безысходностью....

Собственно вот именно поэтому проще и правильнее для таких задач использовать FormArray: не нужно хранить кол-во динамических полей, не нужно понимать индексы таких полей, не нужно придумывать маппинги, и тд.

Если проблематика все еще не совсем прозрачна, то вот вам задачка:
1. Есть форма на которой контролы: фио и навыки
2. Контрол навыки это `input type=text`, навыком может быть бесконечное кол-во. Этот контрол добавляется по кнопке Добавить навык. Начальное кол-во навыков на форме = 2
3. Нужно реализовать эту форму только через FormGroup и FormControl, не используя FormArray
4. Навыки так же можно удалять из списка, через кнопку Удалить навык
5. На сервер должен уйти вот такой обьект:
...
```
6. Когда инфа будет приходить с сервера, нам нужно отрендерить скилы в том порядке в котором они находятся в массиве

После этого тот же самый пример попробуйте реализовать с помощью FormArray. Какой способ проще, гибче и читабельнее?
